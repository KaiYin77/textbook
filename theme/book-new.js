...
(function controllEditMode() {
    let editToggleButton, deleteButton, contentWrap, originalContent;
    ...
    function initialize() {
        editToggleButton = document.getElementById('edit-button');
    ...
    function attachEventListeners() {
        editToggleButton.addEventListener('click', handleEditToggleClick);
    ...
    function handleEditToggleClick() {
        isEditMode ? saveChanges() : loadContentAndEnterEditMode();
    }
    ...
    function enterEditMode() {
        isEditMode = true;
        const markdownContent = htmlToMarkdown(contentWrap.innerHTML);
    ...
    function htmlToMarkdown(html) {
        return html
            ...
            .replace(/&#39;/g, "'")
            .replace(/^\s+|\s+$/g, '')
            .replace(/<h1.*?><a.*?>(.*?)<\/a><\/h1>/gi, '# $1')
            .replace(/<h2.*?><a.*?>(.*?)<\/a><\/h2>/gi, '## $1')
            .replace(/<h3.*?>(.*?)<\/h3>/gi, '### $1')
            .replace(/<h4.*?>(.*?)<\/h4>/gi, '#### $1')
            .replace(/<h5.*?>(.*?)<\/h5>/gi, '##### $1')
            .replace(/<h6.*?>(.*?)<\/h6>/gi, '###### $1')
            ...
            .replace(/<pre><code>\s*([\s\S]*?)\s*<\/code><\/pre>/gi, '```\n$1\n```')
            .replace(/<code>([\s\S]*?)<\/code>/gi, '`$1`')
            .replace(/<hr\s*\/?>/gi, '---')
            .replace(/<[^>]+>\s*([\s\S]*?)\s*<\/[^>]+>/g, '$1');
    }

    function updateEditButtonStyle() {
        editToggleButton.title = isEditMode ? 'Save changes' : 'Edit';
        editToggleButton.setAttribute('aria-label', isEditMode ? 'Save changes' : 'Edit');
        editToggleButton.innerHTML = isEditMode ? '<i class="far fa-save"></i>' : '<i class="far fa-edit"></i>';
    }
    ...
    async function saveChanges() {
        ...
        const editor = contentWrap.querySelector('textarea');
        if (!editor) {
            console.error('Editor not found');
            return;
        }

        const filePath = window.location.pathname.replace(/^\//, '').replace(/\.html$/, '.md');

        try {
            const response = await fetch('http://localhost:8000/save-content', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ content: editor.value, file_path: filePath }),
            });
            if (response.ok) {
                exitEditMode();
            } else {
    ...
    function updateDeleteButton() {
        ...
        rightButtons.style.width = isEditMode ? '96px' : '48px';
        rightButtons.style.gap = isEditMode ? '0px' : '8px';
        deleteButton.style.display = isEditMode ? '' : 'none';
    }
...
function createPost() {
    ...
    const [filename, title] = filenameInput.value.split('/');

    if (!filename || !filename.endsWith('.md') || !title) {
        filenameInput.style.border = '1.5px dashed rgba(255, 0, 0, 0.6)';
        return;
    }
...
async function sendCreatePost(filename, title) {
    try {
        const response = await fetch('http://localhost:8000/create-post', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ file_path: filename, title: title }),
        });
        if (response.ok) {
